...
from cl_model_reg import KnowSynergyCL, BioEncoder, HgnnEncoder, Decoder
...
def train_cl(drug_set, cline_set, edge_data, index, label_train, alpha):
    ...
    for batch, (drug, cline) in enumerate(zip(drug_set, cline_set)):
        ...
        return [rmse, r2, pr], loss_train, cl_loss, pred_loss
...
def test_cl(drug_set, cline_set, synergy_adj, index, label):
    ...
    with torch.no_grad():
        for batch, (drug, cline) in enumerate(zip(drug_set, cline_set)):
            drug = drug.to(device)
            pred, project_embed = cl_model(drug.x, drug.edge_index, drug.batch, cline[0], synergy_adj,
                                            index[:, 0], index[:, 1], index[:, 2])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save', type=str, default="reg_result")
    ...
    best_val_metric = [0, 0, 0]
    best_val_test_metric = [0, 0, 0]
    best_test_metric = [0, 0, 0]
    best_test_val_metric = [0, 0, 0]
    best_val_epoch = 0
    best_test_epoch = 0

    logger.info('Metric: RMSE, R2, Pearson r')
    for epoch in range(1,epochs+1):
        ...
        # ---5CV
        final_test_metric = np.zeros(3)
        final_val_metric = np.zeros(3)
        fold_num = 0
        train_loss_all = 0
        cl_loss_all = 0
        pred_loss_all = 0
        ...
        cl_loss_all /=5
        pred_loss_all /=5
        if final_test_metric[2] > best_test_metric[2]:
            best_test_epoch = epoch
            best_test_metric = final_test_metric
            best_test_val_metric = final_val_metric
        if final_val_metric[2] > best_val_metric[2]:
            best_val_epoch = epoch
            best_val_metric = final_val_metric
            best_val_test_metric = final_test_metric
        logger.info('Epoch:{:04d}, train_loss:{:.6f}, cl_loss:{:.6f}, pred_loss:{:.6f}, '.format(epoch, train_loss_all, cl_loss_all, pred_loss_all)+'val: {:.6f} '.format(final_val_metric[0])+
            '{:.6f} '.format(final_val_metric[1])+
            '{:.6f} '.format(final_val_metric[2]))
